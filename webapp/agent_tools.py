...
def query_facilities(snapshot: Dict[str, Any], name: str = "", facility_type: str = "",
    ...
    return {
        ...
        "分級門檻": "基礎健康分數＝100−(D×0.45+E×0.2+R×0.35)×21，限[15,95]；A1依平台規則校正為健康90分；風險分＝100−健康分數；"
                    "風險分≥75或U4→高風險，≥50或U3→中風險，其餘低風險。"
                    "A1＝D0/E1/R1・U1，健康90分、風險10分，屬低風險。",
        "設施": out[:20],
        "已截斷": len(out) > 20,
    }
...
DOC_SCOPES: Dict[str, Dict[str, Any]] = {
    ...
    "流域生態背景": {
        "說明": "石虎族群監測、大雪山黑熊監測、野生動物資源保育、流域社區友善環境計畫",
        #  檔名比對用。2026-08-30 加入黑熊監測簡報，原關鍵字只涵蓋石虎，
        #  「大雪山黑熊」那份會被 scope 過濾整批濾掉。
        "keywords": ("石虎", "黑熊", "瀕危物種", "野生動物資源",
                     "保育自主管理", "友善環境產業", "保育軸帶"),
    },
}

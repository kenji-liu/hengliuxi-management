...
QUERY_CONCEPT_TERMS: Dict[str, Tuple[str, ...]] = {
    ...
    #  哺乳類與陸域野生動物。對應 2026-08-30 加入的六份文件（613 段）：
    #  石虎族群監測期中報告與審查簡報、東勢處野生動物資源永續利用、
    #  山城社區友善環境產業輔導、大雪山黑熊監測簡報。
    #  原本 'ecology' 只有「動物」「物種」「保育」等泛詞，實測
    #  「臺灣黑熊在大雪山的監測結果」「石虎族群監測計畫OI值」兩題
    #  query_concepts 回傳空集合 → is_hengliuxi_query 判 False，
    #  段落只能落到 fallback_unscored（relevance 全 0，等於沒有排序）。
    'wildlife': ('石虎', '黑熊', '哺乳類', '哺乳動物', '野生動物',
                 '穿山甲', '食蟹獴', '山羌', '野豬', '獼猴', '白鼻心',
                 '自動相機', '紅外線相機', '出現頻率', 'oi值', 'oi 值',
                 '淺山', '保育軸帶', '生態廊道', '棲地連通', '族群監測',
                 '繫放', '頸圈', '狩獵', '獵人', '生態給付', '里山',
                 '大雪山', '烏溪', '大安溪'),
}

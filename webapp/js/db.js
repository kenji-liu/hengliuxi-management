...
const DB = {
  ...
  // 資料版本（每次重大更新設施資料時遞增）
  VERSION: '5.26',  // 匯入115年構造物調查表28筆（宜順工程顧問，115.7.18~7.19）  // 強制重新正規化巡查紀錄：套用sf_grade誤植自癒＋風險分級統一，修正舊快取顯示A1但實際為B/C級的問題  // 復原被「維護管理」刪除鈕誤刪的專業巡查紀錄（id 8200/9009 等）：舊版刪除鈕不分來源一律永久刪除，此版已修正並補回缺漏的 seed 巡查（依 dedupeKey 合併，不影響使用者既有資料）
      ...
      if (!data.settings?.version || data.settings.version !== this.VERSION) {
        ...
        const merged = {
          ...
          inspections: (() => {
            ...
            // 依 dedupe key 建立 seed 索引，供既有記錄回填官方照片
            const seedByKey = new Map();
            defaults.inspections.forEach(r => {
              const key = this._inspectionDedupeKey(r);
              if (key) seedByKey.set(key, r);
            });
            // 回填：既有巡查若無照片、而最新 seed 有官方照片，補上（不覆蓋用戶已存在照片）
            //  另補齊 seed 後續新增的唯讀補充欄位（備註、次要標籤、來源頁碼等）。
            //  遷移採「使用者紀錄優先」，既有紀錄不會被 seed 覆寫，因此新增欄位
            //  必須在此明確回填，否則平台改了資料、使用者端卻看不到。
            //  僅在使用者端「該欄為空」時補值，不覆蓋任何既有內容。
            const FILL_KEYS = ['notes', 'secondaryTags', 'dateCorrected', 'sourcePage',
                               'sourcePdf', 'sourceFormBatch', 'importedFrom', 'photoSource',
                               'defectType', 'repairMethod', 'repairScale', 'repairCost'];
            const enriched = userInspections.map(u => {
              const key = this._inspectionDedupeKey(u);
              const seed = key && seedByKey.get(key);
              if (!seed) return u;
              const out = { ...u };
              if (Array.isArray(seed.photos) && seed.photos.length &&
                  !(Array.isArray(u.photos) && u.photos.length)) {
                out.photos = seed.photos.slice();
              }
              FILL_KEYS.forEach(k => {
                const cur = out[k];
                const isEmpty = cur === undefined || cur === null || cur === '' ||
                                (Array.isArray(cur) && !cur.length);
                if (isEmpty && seed[k] !== undefined && seed[k] !== null && seed[k] !== '') {
                  out[k] = Array.isArray(seed[k]) ? seed[k].slice() : seed[k];
                }
              });
              return out;
            });
            const seen = new Set(enriched.map(r => this._inspectionDedupeKey(r)).filter(Boolean));
  ...
  init() {
    const data = {
      ...
      inspections: [
        ...
        // ── 105年(2016-05-24)構造物巡檢：橫流溪01~04整治工程結構物檢測表（林志勇，附錄一）──
        { id: 8101, facilityId: 13, facilityName: '溪構1-1 粗石斜曲面式魚道', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構1-1', position: '横流溪', structType: '粗石斜曲面式魚道 混凝土砌塊石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪下游整治工程）：粗石斜曲面式魚道外觀良好、功能健全（A1級）。', action: '例行維護（定期檢測）', photos: ['/webapp/assets/report-photos/inspect105/xigou1-1_2016.jpg'] },
        { id: 8102, facilityId: 14, facilityName: '溪構1-2 改良型舟通式魚道', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構1-2', position: '横流溪', structType: '改良式舟通式魚道 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪下游整治工程）：改良式舟通式魚道外觀良好、功能健全（A1級）。', action: '例行維護（定期檢測）', photos: ['/webapp/assets/report-photos/inspect105/xigou1-2_2016.jpg'] },
        { id: 8103, facilityId: 11, facilityName: '溪構9 固床工', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構9', position: '横流溪', structType: '固床工（1號固床工）混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪下游整治工程）：1號固床工外觀良好、功能健全（A1級）。〔對應推定：1號固床工→溪構9，依固床工數量(3座)與由上游至下游序位推定〕', action: '例行維護（定期檢測）', photos: [] },
        { id: 8104, facilityId: 8, facilityName: '溪構10 固床工', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構10', position: '横流溪', structType: '固床工（2號固床工）混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪下游整治工程）：2號固床工外觀良好、功能健全（A1級）。另既設#2固床工修補1處亦檢測良好。〔對應推定：2號固床工→溪構10〕', action: '例行維護（定期檢測）', photos: [] },
        { id: 8105, facilityId: 3, facilityName: '溪構11 階梯式固床工', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構11', position: '横流溪', structType: '固床工（3號固床工）混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪下游整治工程）：3號固床工外觀良好、功能健全（A1級）。〔對應推定：3號固床工→溪構11〕', action: '例行維護（定期檢測）', photos: ['/webapp/assets/report-photos/inspect105/xigou11_2016.jpg'] },
        { id: 8106, facilityId: 6, facilityName: '溪構5-1 防砂壩', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構5-1', position: '横流溪', structType: '防砂壩（既有修復與加高）混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級（A1）進入定期檢測系統', findings: '105年構造物檢測（橫流溪整治第二期工程）：既有防砂壩修復與加高，外觀良好、功能健全（A1級，進入定期檢測系統）。〔對應推定：第二期防砂壩→溪構5-1〕', action: 'A1：進入定期檢測系統', photos: ['/webapp/assets/report-photos/inspect105/fangsha5-1_2016.jpg'] },
        { id: 8107, facilityId: 2, facilityName: '溪構8-1 防砂壩', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－溪構8-1', position: '横流溪', structType: '防砂壩 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測（橫流溪野溪整治第三期工程）：防砂壩外觀良好、功能健全（A1級）。〔對應推定：第三期防砂壩→溪構8-1〕', action: '例行維護（定期檢測）', photos: [] },
        { id: 8108, facilityId: 15, facilityName: '護岸', formType: 'professional_structure', inspector: '林志勇', inspectUnit: '裡冷溪及橫流溪整體治理規劃與監測調查', date: '2016-05-24', weather: '', status: '完成', priority: '低', appearanceOther: '', sourcePdf: '頁面擷取自-裡冷溪及橫流溪整體治理規劃與監測調查-105年構造物巡檢.pdf', inspectNo: '橫流溪－105－護岸', position: '横流溪', structType: '護岸／護坡類（漿砌石‧乾砌石‧重力式‧半重力式‧混凝土砌塊石‧護岸加高‧木排樁護坡‧槽框保護工）', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A1級 例行維護（定期檢測）', findings: '105年構造物檢測：橫流溪各期整治工程之護岸／護坡類構造物均外觀良好、功能健全（A1級）。含漿砌石護坡、木排樁護坡（野溪整治工程）；重力式護岸、混凝土砌塊石護岸、既有護岸加高、乾砌石護坡（第二期）；乾砌石護坡、混砌石護岸（第三期）；半重力式護岸、槽框保護工（下游整治工程）。', action: '例行維護（定期檢測）', photos: ['/webapp/assets/report-photos/inspect105/hugan_2016.jpg'] },
        // ── 114~115年 一般性定期巡查（護管員例行巡查，附錄一表單）──
        { id: 9001, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2025-11-26', inspector: '張森源、汪竣泰', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '晴', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常', action: '繼續定期巡查', status: '完成', priority: '低', photos: ["/webapp/assets/inspection-photos/general/2025-11-26/01.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/02.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/03.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/04.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/05.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/06.jpeg", "/webapp/assets/inspection-photos/general/2025-11-26/07.jpeg"] },
        { id: 9002, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2025-12-30', inspector: '陳英吉、汪竣泰', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '晴', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常', action: '繼續定期巡查', status: '完成', priority: '低', photos: ["/webapp/assets/inspection-photos/general/2025-12-30/01.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/02.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/03.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/04.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/05.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/06.jpeg", "/webapp/assets/inspection-photos/general/2025-12-30/07.jpeg"] },
        { id: 9003, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2026-01-29', inspector: '張森源、汪竣泰', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施發現■結構外觀破損（座標24.186274°N,120.909574°E），需通知分署及專業技師評估；告示牌/解說牌正常；救生圈正常', action: '異常通報：魚道/防砂設施結構外觀破損，通知分署安排專業評估', status: '待處理', priority: '中', photos: ["/webapp/assets/inspection-photos/general/2026-01-29/01.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/02.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/03.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/04.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/05.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/06.jpeg", "/webapp/assets/inspection-photos/general/2026-01-29/07.jpeg"] },
        { id: 9004, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2026-02-24', inspector: '張森源、陳英吉', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常', action: '繼續定期巡查', status: '完成', priority: '低', photos: ["/webapp/assets/inspection-photos/general/2026-02-24/01.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/02.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/03.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/04.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/05.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/06.jpeg", "/webapp/assets/inspection-photos/general/2026-02-24/07.jpeg"] },
        { id: 9005, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2026-03-31', inspector: '張森源、汪竣泰', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常', action: '繼續定期巡查', status: '完成', priority: '低', photos: ["/webapp/assets/inspection-photos/general/2026-03-31/01.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/02.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/03.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/04.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/05.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/06.jpeg", "/webapp/assets/inspection-photos/general/2026-03-31/07.jpeg"] },
        { id: 9006, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'general_periodic', date: '2026-04-29', inspector: '張森源、汪竣泰', inspectUnit: '林業保育署臺中分署麗陽工作站', weather: '', findings: '步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常', action: '繼續定期巡查', status: '完成', priority: '低', photos: ["/webapp/assets/inspection-photos/general/2026-04-29/01.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/02.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/03.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/04.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/05.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/06.jpeg", "/webapp/assets/inspection-photos/general/2026-04-29/07.jpeg"] },
        // ── OCR 補匯入（影像型 PDF 判讀）──
        ...
        // ── 113年(2024-07-03)全面檢測：工程13/19/71/31 構造物調查表（余建廷、張哲銘）──
        // PDF 13 - 橫流溪野溪下游整治工程（104年竣工）
        { id: 9010, facilityId: 13, facilityName: '溪構1-1 粗石斜曲面式魚道', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構1-1－13', position: '横流溪下游段', structType: '粗石斜曲面式魚道 鋼筋混凝土植石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '粗石斜曲面式魚道(L=36.08m, W=21.41m, 鋼筋混凝土植石)，外觀狀況良好，功能健全，無裂縫、磨蝕、淘空或傾倒等劣化情形。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', sourcePage: '13-4', photos: ['/webapp/assets/report-photos/inspect113/xigou1-1_2024_1.jpg', '/webapp/assets/report-photos/inspect113/xigou1-1_2024_2.jpg'] },
        { id: 9011, facilityId: 14, facilityName: '溪構1-2 改良型舟通式魚道', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構1-2－13', position: '横流溪下游段', structType: '改良式舟通魚道 鋼筋混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '改良式舟通魚道(L=40.46m, W=3.00m, 鋼筋混凝土)，外觀狀況良好，功能健全，無劣化情形。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', sourcePage: '13-4', photos: ['/webapp/assets/report-photos/inspect113/xigou1-2_2024_1.jpg'] },
        { id: 9012, facilityId: 15, facilityName: '護岸', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－護岸－13', position: '横流溪下游段', structType: '半重力式護岸 混凝土乾砌石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（不定期檢測）', findings: '半重力式護岸(L=180m)、護岸加高(I)(L=58.59m)、護岸加高(II)(L=47.11m)、護岸基礎補強(I-IV)、牆體修復工，外觀良好，功能健全，無明顯劣化。', action: '例行維護（不定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', sourcePage: '13-3/13-4', photos: ['/webapp/assets/report-photos/inspect113/hugan13_2024_1.jpg', '/webapp/assets/report-photos/inspect113/hugan13_2024_2.jpg', '/webapp/assets/report-photos/inspect113/hugan13_2024_3.jpg', '/webapp/assets/report-photos/inspect113/hugan13_2024_4.jpg'] },
        { id: 9013, facilityId: 11, facilityName: '溪構9 固床工', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構9－13', position: '横流溪下游段', structType: '固床工 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '#1固床工(L=27.54m, H=1.7m)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou9-13_2024_1.jpg'] },
        { id: 9014, facilityId: 8, facilityName: '溪構10 固床工', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構10－13', position: '横流溪下游段', structType: '固床工 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '#2固床工(L=23.58m, H=1.7m)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou10-13_2024_1.jpg'] },
        { id: 9015, facilityId: 3, facilityName: '溪構11 階梯式固床工', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構11－13', position: '横流溪下游段', structType: '固床工 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '#3固床工(L=30.23m, H=2.6m)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '13_橫流溪野溪下游整治工程.pdf', sourcePage: '13-3', photos: ['/webapp/assets/report-photos/inspect113/xigou11-13_2024_1.jpg'] },
        // PDF 19 - 橫流溪下游整治第二期工程（105年竣工）
        { id: 9016, facilityId: 5, facilityName: '溪構6 階段式魚道', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構6－19', position: '横流溪下游段', structType: '階梯式魚道 混凝土塊石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '階梯式魚道(L=18.50m, H=1.7m)，外觀狀況良好，功能健全，無劣化情形。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '19_橫流溪下游整治第二期工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou6-19_2024_1.jpg', '/webapp/assets/report-photos/inspect113/xigou6-19_2024_2.jpg'] },
        { id: 9017, facilityId: 15, facilityName: '護岸', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－護岸－19', position: '横流溪下游段', structType: '護岸 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（不定期檢測）', findings: 'A型護岸(L=69m, H=5.4m)、B型護岸(L=49.5m)、護岸加高(W=1.0m, H=2.2m)，外觀良好，功能健全。', action: '例行維護（不定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '19_橫流溪下游整治第二期工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/hugan19_2024_1.jpg', '/webapp/assets/report-photos/inspect113/hugan19_2024_2.jpg', '/webapp/assets/report-photos/inspect113/hugan19_2024_3.jpg'] },
        // PDF 71 - 橫流溪下游段改善工程（108年竣工）
        { id: 9018, facilityId: 1, facilityName: '溪構8-2 之字形魚道', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構8-2－71', position: '横流溪 0K+460', structType: '之字形魚道 鋼筋混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '0K+460魚道(W=23.3m, H=14.5m, 鋼筋混凝土)及0K+520魚道出口保護設施(L=12.0m, H=2.7m)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '71_橫流溪下游段改善工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou8-2_2024_1.jpg', '/webapp/assets/report-photos/inspect113/xigou8-2_2024_2.jpg'] },
        { id: 9019, facilityId: 5, facilityName: '溪構6 階段式魚道', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構6－71', position: '横流溪 1K+161~1K+327', structType: '階段式魚道 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '1K+161魚道改善(L=24.7m, H=2.0m)及1K+327魚道改善(L=15.0m, H=0.2m)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '71_橫流溪下游段改善工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou6-71_2024_1.jpg', '/webapp/assets/report-photos/inspect113/xigou6-71_2024_2.jpg'] },
        { id: 9020, facilityId: 3, facilityName: '溪構11 階梯式固床工', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－溪構11－71', position: '横流溪上游段', structType: '階梯式固床工 漿砌石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '階梯式固床工(L=10.5m, W=39.0m, 漿砌石)，外觀狀況良好，功能健全，無裂縫或淘空情形。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '71_橫流溪下游段改善工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/xigou11-71_2024_1.jpg', '/webapp/assets/report-photos/inspect113/xigou11-71_2024_2.jpg'] },
        { id: 9021, facilityId: 15, facilityName: '護岸', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－護岸－71', position: '横流溪', structType: '護岸基礎補強 混凝土', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '護岸基礎補強(L=30.0m, H=4.5m, 混凝土)，外觀狀況良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '71_橫流溪下游段改善工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/hugan71_2024_1.jpg', '/webapp/assets/report-photos/inspect113/hugan71_2024_2.jpg', '/webapp/assets/report-photos/inspect113/hugan71_2024_3.jpg'] },
        // PDF 31 - 仙區23林班治理工程（105年竣工）
        { id: 9022, facilityId: 19, facilityName: '平臺3', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－平臺3－31', position: '横流溪上游親水區', structType: '景觀平台 南洋櫸木H型鋼鍍鋅方管', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '景觀平台(L=60m, W=40.5m, 南洋櫸木H型鋼鍍鋅方管)，外觀狀況良好，功能健全，木材無明顯腐朽，鋼構無鏽蝕。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '31_仙區23林班治理工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/pingtai3-31_2024_1.jpg', '/webapp/assets/report-photos/inspect113/pingtai3-31_2024_2.jpg'] },
        { id: 9023, facilityId: 20, facilityName: '平臺4', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－平臺4－31', position: '横流溪上游親水區', structType: '景觀平台 南洋櫸木H型鋼鍍鋅方管', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '景觀平台(L=60m, W=40.5m, 南洋櫸木H型鋼鍍鋅方管)，外觀狀況良好，功能健全，木材無明顯腐朽，鋼構無鏽蝕。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '31_仙區23林班治理工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/pingtai4-31_2024_1.jpg', '/webapp/assets/report-photos/inspect113/pingtai4-31_2024_2.jpg'] },
        { id: 9024, facilityId: 16, facilityName: '橫流溪全段（步道0K+000~1K+290）', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－步道護欄－31', position: '横流溪上游親水區', structType: '護岸護欄 南洋櫸木鍍鋅鋼板', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（不定期檢測）', findings: 'A型護岸護欄(L=95.5m, H=1.2m)、B型(L=9.0m, H=1.3m)、C型(L=18.0m, H=1.2m)、D型(L=8.5m, H=1.2m)，南洋櫸木鍍鋅鋼板，外觀良好，無明顯腐朽或鬆脫。', action: '例行維護（不定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '31_仙區23林班治理工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/budao-rail31_2024_1.jpg', '/webapp/assets/report-photos/inspect113/budao-rail31_2024_2.jpg', '/webapp/assets/report-photos/inspect113/budao-rail31_2024_3.jpg'] },
        { id: 9025, facilityId: 15, facilityName: '護岸', formType: 'professional_structure', date: '2024-07-03', inspector: '余建廷、張哲銘', inspectUnit: '林業及自然保育署臺中分署', inspectNo: '橫流溪－113－護岸－31', position: '横流溪上游親水區', structType: '擋土牆護坡 鋼筋混凝土砌石', level: 'A級', icLevel: null, deru_d: 0, deru_e: 1, deru_r: 1, deru_u: 1, deru_label: 'A級 例行維護（定期檢測）', findings: '擋土牆護坡工程(L=95.5m, H=1.6m, RC)、乾砌石擋土牆(L=77m)、階段式砌石擋土牆漸變(L=45m)、B型擋土牆(L=20m, RC)、階段式砌石擋土牆(L=132m, H=1.5m)，外觀良好，功能健全。', action: '例行維護（定期檢測）', appearanceOther: '', status: '完成', priority: '低', weather: '晴', sourcePdf: '31_仙區23林班治理工程.pdf', photos: ['/webapp/assets/report-photos/inspect113/hugan-wall31_2024_1.jpg', '/webapp/assets/report-photos/inspect113/hugan-wall31_2024_2.jpg', '/webapp/assets/report-photos/inspect113/hugan-wall31_2024_3.jpg'] },
      
        // ── 115年(2026-07-18/19)構造物調查表：28筆，宜順工程顧問，逐頁判讀自
        //    115維護管理/構造物調查表_簽.pdf（A級23、B1-I 3、B1-III 1、B2 1）──
        {"id": 8200, "facilityId": 13, "facilityName": "溪構1-1 粗石斜曲面式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構1-1", "position": "橫流溪1K+400", "structType": "防砂設施、粗石斜曲面式魚道", "level": "B級,B1-I級", "deru_label": "B1-I級 例行處理維護", "findings": "整體現況大致良好，惟下游右側部份格框消能設施基礎淘空，建議進行基礎修補", "action": "例行處理維護", "status": "待處理", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 2, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構1-1/01.jpg", "/webapp/assets/inspection-photos/115/溪構1-1/02.jpg", "/webapp/assets/inspection-photos/115/溪構1-1/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 2, "deru_e": 1, "deru_r": 2, "defectType": "基礎淘空", "appearanceOther": "下游部份格框消能設施基礎淘空", "repairMethod": "格框消能設施基礎修補", "repairScale": "24m2", "repairCost": 50},
        {"id": 8201, "facilityId": 14, "facilityName": "溪構1-2 改良型舟通式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構1-2", "position": "橫流溪1K+400", "structType": "改良型舟通式魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體外觀狀況良好，建議例行維護(即定期檢測)。", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 5, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構1-2/01.jpg", "/webapp/assets/inspection-photos/115/溪構1-2/02.jpg", "/webapp/assets/inspection-photos/115/溪構1-2/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8202, "facilityId": 12, "facilityName": "溪構2 階段式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構2", "position": "橫流溪1K+315", "structType": "階段式魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體現況良好。", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 8, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構2/01.jpg", "/webapp/assets/inspection-photos/115/溪構2/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8203, "facilityId": 10, "facilityName": "溪構3 斜坡式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構3", "position": "橫流溪1K+225", "structType": "斜坡式魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體現況良好。", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 11, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構3/01.jpg", "/webapp/assets/inspection-photos/115/溪構3/02.jpg", "/webapp/assets/inspection-photos/115/溪構3/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8204, "facilityId": 9, "facilityName": "溪構4 階段式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構4", "position": "橫流溪1K+170", "structType": "階段式魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "整體現況大致良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 14, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構4/01.jpg", "/webapp/assets/inspection-photos/115/溪構4/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8205, "facilityId": 6, "facilityName": "溪構5-1 防砂壩", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構5-1", "position": "橫流溪1K+000", "structType": "防砂壩", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "整體現況良好；縱向廊道以溪構5-2維持", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 17, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構5-1/01.jpg", "/webapp/assets/inspection-photos/115/溪構5-1/02.jpg", "/webapp/assets/inspection-photos/115/溪構5-1/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8206, "facilityId": 7, "facilityName": "溪構5-2 潛越式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構5-2", "position": "橫流溪1K+000", "structType": "潛越式魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 20, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構5-2/01.jpg", "/webapp/assets/inspection-photos/115/溪構5-2/02.jpg", "/webapp/assets/inspection-photos/115/溪構5-2/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8207, "facilityId": 5, "facilityName": "溪構6 階段式魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構6", "position": "橫流溪0K+740", "structType": "階段式魚道", "level": "B級,B1-I級", "deru_label": "B1-I級 例行處理維護", "findings": "整體現況大致良好，惟下游右側部份格框消能設施基礎淘空，建議進行基礎修補", "action": "例行處理維護", "status": "待處理", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 23, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構6/01.jpg", "/webapp/assets/inspection-photos/115/溪構6/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 2, "deru_e": 1, "deru_r": 2, "defectType": "基礎淘空", "appearanceOther": "部份格框消能設施基礎淘空", "repairMethod": "格框消能設施基礎修補", "repairScale": "16m2", "repairCost": 50},
        {"id": 8208, "facilityId": 4, "facilityName": "溪構7 降壩魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構7", "position": "橫流溪0K+560", "structType": "降壩魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 26, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構7/01.jpg", "/webapp/assets/inspection-photos/115/溪構7/02.jpg", "/webapp/assets/inspection-photos/115/溪構7/03.jpg", "/webapp/assets/inspection-photos/115/溪構7/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8209, "facilityId": 2, "facilityName": "溪構8-1 防砂壩", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構8-1", "position": "橫流溪0K+460", "structType": "防砂壩", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體現況良好；縱向廊道以溪構8-2維持", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 29, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構8-1/01.jpg", "/webapp/assets/inspection-photos/115/溪構8-1/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8210, "facilityId": 1, "facilityName": "溪構8-2 之字形魚道", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構8-2", "position": "橫流溪0K+460", "structType": "之字形魚道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物本體結構良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 32, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構8-2/01.jpg", "/webapp/assets/inspection-photos/115/溪構8-2/02.jpg", "/webapp/assets/inspection-photos/115/溪構8-2/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "secondaryTags": ["魚道現況"], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8211, "facilityId": 11, "facilityName": "溪構9 固床工", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構9", "position": "橫流溪1K+265", "structType": "固床工", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體大致良好，溢口有輕微磨蝕情形，建議例行維護(即定期檢測)", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 35, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構9/01.jpg"], "photoCount": 1, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8212, "facilityId": 8, "facilityName": "溪構10 固床工", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構10", "position": "橫流溪1K+040", "structType": "固床工", "level": "B級,B1-I級", "deru_label": "B1-I級 例行處理維護", "findings": "構造物整體大致良好，部分鋼條有位移情形，建議進行修補", "action": "例行處理維護", "status": "待處理", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 38, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構10/01.jpg", "/webapp/assets/inspection-photos/115/溪構10/02.jpg", "/webapp/assets/inspection-photos/115/溪構10/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 3, "deru_e": 1, "deru_r": 3, "defectType": "鋼條位移", "appearanceOther": "連接溪構5-2魚道保護工部分鋼條位移", "repairMethod": "魚道保護工頂部鋼條修補", "repairScale": "-", "repairCost": 20, "notes": "原巴威颱風報告圖1 將本項鋼條位移標註為 114.04.18，經確認為誤植，實際檢測日期為 115.07.18，已更正。", "dateCorrected": "原報告圖1誤植 114.04.18 → 更正為 115.07.18"},
        {"id": 8213, "facilityId": 3, "facilityName": "溪構11 階梯式固床工", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－溪構11", "position": "橫流溪0K+510", "structType": "階梯式固床工", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "構造物整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 41, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/溪構11/01.jpg", "/webapp/assets/inspection-photos/115/溪構11/02.jpg", "/webapp/assets/inspection-photos/115/溪構11/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8214, "facilityId": 15, "facilityName": "護岸", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+510護岸", "position": "橫流溪0K+510", "structType": "護岸", "level": "B級,B1-III級", "deru_label": "B1-III級 一年內必須處理維護", "findings": "既有涵管遭沖毀，致使護岸背填土方流失，建議進行補強並回填背填土", "action": "一年內必須處理維護", "status": "待處理", "priority": "高", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 45, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K510護岸/01.jpg", "/webapp/assets/inspection-photos/115/0K510護岸/02.jpg", "/webapp/assets/inspection-photos/115/0K510護岸/03.jpg", "/webapp/assets/inspection-photos/115/0K510護岸/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 3, "deru_e": 3, "deru_r": 3, "defectType": "背填土方流失", "appearanceOther": "既有涵管遭沖毀，致使護岸背填土方流失（分析時 alpha 值取 2）", "repairMethod": "護岸補強並回填背填土", "repairScale": "3.5*5*1.7m", "repairCost": 100},
        {"id": 8215, "facilityId": 15, "facilityName": "護岸", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－1K+000護岸", "position": "橫流溪1K+000", "structType": "護岸", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "護岸整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 48, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/1K000護岸/01.jpg", "/webapp/assets/inspection-photos/115/1K000護岸/02.jpg", "/webapp/assets/inspection-photos/115/1K000護岸/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8216, "facilityId": 15, "facilityName": "護岸", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－1K+170護岸", "position": "橫流溪1K+170", "structType": "護岸", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "護岸整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 51, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/1K170護岸/01.jpg", "/webapp/assets/inspection-photos/115/1K170護岸/02.jpg", "/webapp/assets/inspection-photos/115/1K170護岸/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8217, "facilityId": 15, "facilityName": "護岸", "formType": "professional_structure", "date": "2026-07-18", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－1K+260護岸", "position": "橫流溪1K+260", "structType": "護岸", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "護岸整體現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 54, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/1K260護岸/01.jpg", "/webapp/assets/inspection-photos/115/1K260護岸/02.jpg", "/webapp/assets/inspection-photos/115/1K260護岸/03.jpg", "/webapp/assets/inspection-photos/115/1K260護岸/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8218, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+000步道", "position": "步道0K+000", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "道路現況人員及車輛可正常通過，建議定期檢測", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 58, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K000步道/01.jpg", "/webapp/assets/inspection-photos/115/0K000步道/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8219, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+370步道", "position": "步道0K+370", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "PC道路現況人員及車輛可正常通過，建議定期檢測", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 61, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K370步道/01.jpg", "/webapp/assets/inspection-photos/115/0K370步道/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8220, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+400步道", "position": "步道0K+400", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "道路現況整體良好，邊坡樹木倒塌，建議進行清除即可", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 64, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K400步道/01.jpg", "/webapp/assets/inspection-photos/115/0K400步道/02.jpg", "/webapp/assets/inspection-photos/115/0K400步道/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8221, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+600步道", "position": "步道0K+600", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "道路現況人員及車輛可正常通行", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 67, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K600步道/01.jpg", "/webapp/assets/inspection-photos/115/0K600步道/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1, "notes": "巴威颱風不定期巡查報告第8頁載明：步道0K+600處之上邊坡崩塌裸露，因鄰近步道且邊坡陡峭裸露，易有落石或崩塌之虞，建議設置擋土牆並刨除危石。惟同報告表3、表5與本調查表功能評估均列為A級，兩者不一致，本欄依原始文件如實併陳，實際處置以現地複核為準。", "secondaryTags": ["邊坡崩塌待複核"]},
        {"id": 8222, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+800步道", "position": "步道0K+800", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "道路現況人員及車輛可正常通行", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 70, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/0K800步道/01.jpg", "/webapp/assets/inspection-photos/115/0K800步道/02.jpg", "/webapp/assets/inspection-photos/115/0K800步道/03.jpg", "/webapp/assets/inspection-photos/115/0K800步道/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8223, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－1K+200步道", "position": "步道1K+200", "structType": "步道", "level": "B級,B2級", "deru_label": "B2級 1～3年內應處理維護", "findings": "木屑步道表層沖蝕，建議配合倒木進行刨木作業並重新鋪設", "action": "1～3年內應處理維護", "status": "待處理", "priority": "中", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 73, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/1K200步道/01.jpg", "/webapp/assets/inspection-photos/115/1K200步道/02.jpg"], "photoCount": 2, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "defectType": "表層沖蝕", "repairMethod": "配合周邊倒木進行刨木作業並重新鋪設", "repairScale": "6m2", "repairCost": 10},
        {"id": 8224, "facilityId": 17, "facilityName": "平臺1", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－平臺1", "position": "橫流溪1K+400", "structType": "平臺", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "平臺現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 77, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/平臺1/01.jpg", "/webapp/assets/inspection-photos/115/平臺1/02.jpg", "/webapp/assets/inspection-photos/115/平臺1/03.jpg", "/webapp/assets/inspection-photos/115/平臺1/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8225, "facilityId": 18, "facilityName": "平臺2", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－平臺2", "position": "橫流溪1K+400", "structType": "平臺", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "平臺現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 80, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/平臺2/01.jpg"], "photoCount": 1, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8226, "facilityId": 19, "facilityName": "平臺3", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－平臺3", "position": "橫流溪1K+315", "structType": "平臺", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "平臺現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 83, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/平臺3/01.jpg", "/webapp/assets/inspection-photos/115/平臺3/02.jpg", "/webapp/assets/inspection-photos/115/平臺3/03.jpg", "/webapp/assets/inspection-photos/115/平臺3/04.jpg"], "photoCount": 4, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},
        {"id": 8227, "facilityId": 20, "facilityName": "平臺4", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－平臺4", "position": "橫流溪1K+225", "structType": "平臺", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "平臺現況良好", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 86, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": ["/webapp/assets/inspection-photos/115/平臺4/01.jpg", "/webapp/assets/inspection-photos/115/平臺4/02.jpg", "/webapp/assets/inspection-photos/115/平臺4/03.jpg"], "photoCount": 3, "photoSource": "115年構造物調查表 PDF 現況照片（依版面裁切）", "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},

        // ── 115年一般性定期巡查 3 筆（11506/11507/11508，含現場照片）──
        {"id": 9101, "facilityId": 16, "facilityName": "橫流溪全段（步道0K+000~1K+290）", "formType": "general_periodic", "date": "2026-06-12", "rocYear": 115, "inspector": "陳英吉、汪竣泰", "inspectUnit": "林業保育署臺中分署麗陽工作站", "weather": "", "findings": "步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常", "action": "繼續定期巡查", "status": "完成", "priority": "低", "sourcePdf": "115維護管理/一般性定期巡查表單11506.pdf", "sourceFormBatch": "115年一般性定期巡查表單", "importedFrom": "115維護管理 一般性定期巡查表單（含內嵌現場照片）", "photos": ["/webapp/assets/inspection-photos/general/2026-06-12/01.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/02.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/03.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/04.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/05.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/06.jpeg", "/webapp/assets/inspection-photos/general/2026-06-12/07.jpeg"], "photoCount": 7, "photoSource": "一般性定期巡查表單 PDF 現場照片（7 項設施各一張）"},
        {"id": 9102, "facilityId": 16, "facilityName": "橫流溪全段（步道0K+000~1K+290）", "formType": "general_periodic", "date": "2026-07-06", "rocYear": 115, "inspector": "張森源、陳英吉", "inspectUnit": "林業保育署臺中分署麗陽工作站", "weather": "", "findings": "步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常", "action": "繼續定期巡查", "status": "完成", "priority": "低", "sourcePdf": "115維護管理/一般性定期巡查表單11507.pdf", "sourceFormBatch": "115年一般性定期巡查表單", "importedFrom": "115維護管理 一般性定期巡查表單（含內嵌現場照片）", "photos": ["/webapp/assets/inspection-photos/general/2026-07-06/01.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/02.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/03.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/04.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/05.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/06.jpeg", "/webapp/assets/inspection-photos/general/2026-07-06/07.jpeg"], "photoCount": 7, "photoSource": "一般性定期巡查表單 PDF 現場照片（7 項設施各一張）"},
        {"id": 9103, "facilityId": 16, "facilityName": "橫流溪全段（步道0K+000~1K+290）", "formType": "general_periodic", "date": "2026-08-17", "rocYear": 115, "inspector": "張森源、汪竣泰", "inspectUnit": "林業保育署臺中分署麗陽工作站", "weather": "", "findings": "步道正常；邊坡正常；平臺/護欄正常；護岸正常；魚道/防砂設施正常；告示牌/解說牌正常；救生圈正常", "action": "繼續定期巡查", "status": "完成", "priority": "低", "sourcePdf": "115維護管理/一般性定期巡查表單11508.pdf", "sourceFormBatch": "115年一般性定期巡查表單", "importedFrom": "115維護管理 一般性定期巡查表單（含內嵌現場照片）", "photos": ["/webapp/assets/inspection-photos/general/2026-08-17/01.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/02.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/03.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/04.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/05.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/06.jpg", "/webapp/assets/inspection-photos/general/2026-08-17/07.jpg"], "photoCount": 7, "photoSource": "一般性定期巡查表單 PDF 現場照片（7 項設施各一張）"},
],
      settings: {
        lastUpdate: new Date().toISOString(),
        version: this.VERSION,

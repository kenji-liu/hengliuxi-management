...
const DB = {
  ...
  // 資料版本（每次重大更新設施資料時遞增）
  VERSION: '5.21',  // 匯入115年構造物調查表28筆（宜順工程顧問，115.7.18~7.19）  // 強制重新正規化巡查紀錄：套用sf_grade誤植自癒＋風險分級統一，修正舊快取顯示A1但實際為B/C級的問題
        ...
        const merged = {
          ...
          inspections: (() => {
            ...
            const enriched = userInspections.map(u => {
              ...
              if (seed && Array.isArray(seed.photos) && seed.photos.length &&
                  !(Array.isArray(u.photos) && u.photos.length)) {
                return { ...u, photos: seed.photos.slice(), sourcePage: u.sourcePage || seed.sourcePage };
              }
              return u;
            });
  ...
  init() {
    const data = {
      ...
      inspections: [
        ...
        {"id": 8221, "facilityId": 16, "facilityName": "步道", "formType": "professional_structure", "date": "2026-07-19", "rocYear": 115, "inspector": "張建輝、莊孟矩", "inspectUnit": "宜順工程顧問股份有限公司", "inspectNo": "橫流溪－115－0K+600步道", "position": "步道0K+600", "structType": "步道", "level": "A級", "deru_label": "A級 例行維護（定期檢測）", "findings": "道路現況人員及車輛可正常通行", "action": "例行維護（定期檢測）", "status": "完成", "priority": "低", "weather": "晴", "sourcePdf": "115維護管理/構造物調查表_簽.pdf", "sourcePage": 67, "sourceFormBatch": "115年構造物調查表", "importedFrom": "115維護管理 構造物調查表_簽.pdf 逐頁判讀", "photos": [], "deru_d": 0, "deru_e": 1, "deru_r": 1, "deru_u": 1},

...
const ENGINEERING_BOOKS = [
  ...
  {
    category: '整治規劃設計監造', year: '近年',
    title: '裡冷溪及橫流溪整體治理規劃與監測調查成果報告（橫流溪部分）',
    type: '成果報告', format: 'PDF',
    tags: ['裡冷溪', '整體治理', '監測調查', '橫流溪'],
    summary: '裡冷溪與橫流溪整體治理規劃及監測調查報告，橫流溪部分提供流域整體治理脈絡。',
    path: '01_工程設施維護與資料/其他相關資料/裡冷溪及橫流溪整體治理規劃與監測調查-成果報告-橫流溪部分.pdf'
  },
  {
    category: '整治規劃設計監造', year: '115',
    title: '臺中西部淺山保育軸帶烏溪流域保育軸帶石虎族群監測計畫期中報告',
    type: '期中報告', format: 'PDF',
    tags: ['石虎', '族群監測', '烏溪流域', '淺山保育', '生態廊道', '自動相機'],
    //  原簡介寫「於橫流溪佈設20點」是捏造的：全份報告未出現「橫流溪」三字
    //  （實測 0 次），20 台是溪流環境相機，架在烏溪、筏子溪、大里溪與旱溪。
    //  但「未涵蓋橫流溪一帶」同樣不對 —— 附錄3 的淺山軸帶樣點以座標比對，
    //  CP48（北軸帶，石虎紀錄「有」）距溪構1-1 僅 0.99 公里。
    //  座標換算已用平台自身設施校驗，誤差 0.0~0.1 公尺。
    //  詳見 webapp/data/leopardcat_cameras.json 與
    //  scripts/build_leopardcat_camera_index.py。
    summary: '臺中西部淺山保育軸帶烏溪流域石虎族群監測計畫115年期中報告（研究期：2025年12月–2027年1月）。附錄3淺山保育軸帶40個樣點、附錄1溪流環境20台自動相機，逐點列有TWD97座標與石虎紀錄有無。報告全文未提及橫流溪，但以座標比對，最近樣點CP48（北軸帶、有石虎紀錄）距溪構1-1僅0.99公里，橫流溪10公里內共14個樣點、其中11個有石虎拍攝紀錄，可作為橫流溪棲地連通工程的石虎廊道背景依據。',
    path: '01_工程設施維護與資料/新增報告書/1150430_臺中西部淺山保育軸帶烏溪流域保育軸帶石虎族群監測計畫期中報告(1).pdf'
  },
  {
    category: '整治規劃設計監造', year: '115',
    title: '臺中西部淺山保育軸帶烏溪流域石虎族群監測計畫期中審查簡報',
    type: '審查簡報', format: 'PDF',
    tags: ['石虎', '族群監測', '期中審查', '簡報', '烏溪流域', '淺山保育'],
    summary: '115年石虎族群監測計畫期中審查簡報，配合期中報告審查程序提交，含監測方法、自動相機初步成果與後續計畫說明，可與期中報告對照使用。',
    path: '01_工程設施維護與資料/新增報告書/115石虎期中審查_簡報.pdf'
  },
  {
    category: '整治規劃設計監造', year: '112',
    title: '大安溪與大甲溪野生動物資源永續利用及保育自主管理架構建立',
    type: '成果報告', format: 'PDF',
    tags: ['大安溪', '大甲溪', '野生動物', '保育自主管理', '東勢處', '永續利用'],
    summary: '112年林業及自然保育署臺中分署委辦成果報告，建立大安溪與大甲溪流域野生動物資源永續利用及保育自主管理架構，為橫流溪所在大甲溪流域生態保育整體規劃之背景文件。',
    path: '01_工程設施維護與資料/新增報告書/東勢處大安溪與大甲溪野生動物資源永續利用及保育自主管理.pdf'
  },
  {
    category: '整治規劃設計監造', year: '114',
    title: '臺中市大甲溪流域東勢周圍山城社區友善環境產業輔導計畫（二）成果報告',
    type: '成果報告', format: 'PDF',
    tags: ['大甲溪流域', '東勢', '山城社區', '友善環境', '石虎棲地', '農業輔導'],
    summary: '114年友善環境產業輔導計畫第二期成果報告，推動大甲溪流域東勢周圍山城社區友善石虎棲地之農業轉型，涵蓋生態調查、石虎出現記錄與農業合作推廣，為橫流溪棲地連通之流域保育背景文件。',
    path: '01_工程設施維護與資料/新增報告書/臺中市大甲溪流域東勢周圍山城社區友善環境產業輔導計畫2+成果報告.pdf'
  },
  {
    category: '整治規劃設計監造', year: '113',
    title: '瀕危物種保育行動研討會－大雪山黑熊監測與人熊衝突防範',
    type: '研討會簡報', format: 'PPTX',
    tags: ['臺灣黑熊', '大雪山', '紅外線相機', '繫放追蹤', '人熊衝突', '生態給付'],
    summary: '林業及自然保育署臺中分署113年瀕危物種保育行動研討會簡報。111年5月至113年3月於大雪山及谷關劃設32個4×4km網格、架設42台紅外線自動相機，其中10台拍到黑熊；大雪山遊樂區另自主架設19台，17台拍到黑熊。為橫流溪所在大甲溪流域「臺灣黑熊」此一主要哺乳類物種的原始監測來源。',
    path: '01_工程設施維護與資料/黑熊資料/113瀕危物種保育行動研討會_大雪山黑熊2.1.pptx'
  },
  {
    category: '整治規劃設計監造', year: '115',
    title: '石虎與橫流溪關係及數量資料整理',
    type: '資料整理', format: 'PDF',
    tags: ['石虎', '橫流溪', 'NAA20', 'OI值', '資料口徑', '南勢部落'],
    //  這份是彙整 4 份報告的分析文件，不是原始調查報告。它的價值在於
    //  把「可以講到什麼程度」寫死：1 個樣區不等於 1 隻、OI 不是隻數、
    //  大甲溪統計不可套用到橫流溪。AI 引用石虎數量時應以此為口徑。
    summary: '彙整新增報告書4份PDF對石虎與橫流溪關係的記載（2026年8月31日整理）。核心結論：南勢部落前期相機監測明列「橫流溪NAA20」為石虎拍攝紀錄樣區，這是橫流溪與石虎最直接的證據；但報告未提供該樣點的獨立拍攝事件數或可辨識個體數，因此只能說「至少1個相機樣區有石虎出現紀錄」，不得寫成隻數或推估族群量。另載明OI＝有效照片數÷相機工作時數×1000小時，是相對出現頻度而非個體數，大甲溪流域統計不可直接套用為橫流溪數量。附各項數據的來源報告與頁碼。',
    path: '01_工程設施維護與資料/新增報告書/石虎與橫流溪關係及數量資料整理.pdf'
  },
  {
    category: '整治規劃設計監造', year: '115',
    title: '石虎在橫流溪出沒情況專業深化分析與後續監測建議',
    type: '專業分析', format: 'PDF',
    tags: ['石虎', '橫流溪', 'NAA20', 'CP48', '繁殖訊號', '廊道功能', '監測設計'],
    //  結論分四級（已證實／高度支持／支持性推論／尚待確認），每級都標明
    //  「不可跨越的界線」。AI 回答石虎相關問題時應照這個分級講，
    //  不得把座標鄰近或區域統計說成橫流溪的族群量。
    summary: '12頁專業分析（2026年8月31日）。以四級證據分級判讀：【已證實】南勢部落前期監測於橫流溪NAA20拍攝到石虎；【高度支持】座標比對顯示最近有紀錄樣點CP48距設施0.99公里，10公里內14點中11點有累積紀錄；【支持性推論】CS27（6.09公里）有幼體及疑似懷孕母獸、CP50（7.63公里）有疑似懷孕母獸；【尚待確認】個體數、性別、停留時間與工程影響。另載季節OI（夏0.70／秋0.74／冬0.40／春0.43，Friedman p=0.212未顯著）、犬隻出現比例75%平均OI 8.18為主要風險背景，以及12–16台相機、30分鐘獨立事件規則等後續監測設計建議。結論為「確認出現、支持廊道功能、族群量未知」。',
    path: '01_工程設施維護與資料/新增報告書/石虎在橫流溪出沒情況專業深化分析.pdf'
  },
  {
    category: '整治規劃設計監造', year: '115',
    title: '石虎在橫流溪出沒情況專業深化分析（整合金質獎簡報版）',
    type: '專業分析', format: 'PDF',
    tags: ['石虎', '橫流溪', 'NAA20', 'CP48', '金質獎簡報', '交叉驗證', '國土綠網'],
    //  比前一版多出第五節「金質獎簡報交叉驗證」，逐張說明簡報哪幾頁
    //  可以支持什麼、以及各自的證據限制。這是判讀簡報時的對照表。
    summary: '前一版深化分析的擴充版（14頁），新增「金質獎簡報交叉驗證」專節。逐頁對照簡報投影片7、17、24、37、42、59、74、87可支持的判讀與其限制：投影片7界定維管場域0K+460～1K+400、集水面積2,337公頃、防砂設施5座魚道9座平臺4座；投影片37載明國土生態綠網已盤點指認橫流溪之石虎生態敏感區位；投影片42標題「維管場域發現石虎蹤跡」並附紅外線相機影像；投影片59為近5年77次設施巡查。明確指出簡報的價值是交叉佐證場域出現與管理脈絡，未提供石虎相機資料表，不可新增可計數資料（77次是巡查次數，不是相機夜或石虎事件）。',
    path: '01_工程設施維護與資料/新增報告書/石虎在橫流溪出沒情況專業深化分析_整合金質獎簡報版.pdf'
  }
...
function askEngineeringBookAI(title) {
  ...
  if (input) {
    //  舊提示只寫「請依據『書名』分析工程設計重點」，沒有要求鎖定該份
    //  文件，檢索因此會撈到其他文件的段落（實測點魚道成效追蹤卻回維護
    //  經費表）。這裡明確指定只用這一份，並問出它實際有的內容。
    //  問題數要配合模式輪數：專業問答 max_rounds=3，四問會用光輪數而回空值
    //  （實測 91 秒後回 0 字）。三問可在輪數內完成。
    input.value =
      `請只依據《${title}》這一份文件回答，不要引用其他報告：\n` +
      `1. 調查或設計目的與範圍\n` +
      `2. 方法與主要成果數據（請附頁碼）\n` +
      `3. 與橫流溪設施或生態監測的關聯，以及可引用於維護管理的結論`;
    input.focus();
  }
}

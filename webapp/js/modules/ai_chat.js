...
• 10月：汛後巡查+清淤（確保冬季基流期魚道通水）` },
  ...
  // ── 新增報告書（2025年新入庫） ──
  { id:'leopard_cat_115_interim', kw:['石虎','族群監測','115年','期中','審查','簡報','自動相機','紅外線相機','OI值','烏溪','淺山保育','生態廊道','族群動態'],
    title:'115年石虎族群監測計畫期中審查（臺中西部淺山保育軸帶烏溪流域）',
    body:`資料來源：115年石虎族群監測計畫期中審查簡報（新增報告書/115石虎期中審查_簡報.pdf）
研究計畫：臺中西部淺山保育軸帶烏溪流域保育軸帶石虎族群監測計畫
研究期間：2025-2027年（115-117年），期中報告為研究中期成果
委辦背景：配合農業部林業及自然保育署臺中分署推動之淺山保育軸帶計畫

監測設計：
• 橫流溪調查區域：佈設20個自動相機監測點，覆蓋橫流溪沿岸生態廊道
• 保育軸帶外對照區：設置40個相機點作為對照，合計60點
• 主要指標：石虎出現指數（OI值）= 石虎獨立事件數 ÷ 相機有效工作日數 × 100
• 追蹤方式：個體辨識（斑紋辨識）、活動時段分析

生態連通性意義：
• 橫流溪棲地連通工程（溪流改善、魚道設置）有助於維持石虎廊道連續性
• 自動相機記錄可評估工程施作前後石虎利用頻度變化
• 監測成果可作為大甲溪流域生態廊道保育規劃之科學依據

管理應用：
• 石虎OI值變化應納入橫流溪工程效益評估指標之一
• 巡查人員須注意路殺風險：橫流溪周邊道路若有工程施工，應設置石虎友善通道
• 與113年東勢處友善環境計畫（山城社區）有相互支持關係` },

  { id:'daan_dajia_wildlife_112', kw:['大安溪','大甲溪','野生動物','保育自主管理','永續利用','東勢處','林業署','112年','自主管理架構','生態資源','流域保育'],
    title:'112年大安溪與大甲溪野生動物資源永續利用及保育自主管理架構建立',
    body:`資料來源：東勢處大安溪與大甲溪野生動物資源永續利用及保育自主管理.pdf（121頁）
委辦單位：農業部林業及自然保育署臺中分署（東勢處）
執行單位：計畫顧問公司
計畫主持人：生態保育領域專家
完成時間：民國112年12月

計畫核心目標：
• 建立大安溪與大甲溪流域野生動物資源永續利用及保育自主管理架構
• 整合在地社區、農業生產者與政府機關的保育合作機制
• 促進流域尺度的野生動物棲地連通性與永續利用

主要工作內容：
1. 流域野生動物資源調查：包含哺乳動物（石虎、臺灣獼猴、山羌等）、鳥類、兩棲爬蟲類
2. 自主管理社區組織架構建立：成立大安溪、大甲溪流域保育自主管理協會機制
3. 農民合作模式推廣：研提農業生產與生態保育相容之操作規範
4. 保育教育推廣活動

與橫流溪之關聯：
• 橫流溪位於大甲溪流域，本計畫為大甲溪流域生態保育整體規劃之背景文件
• 本計畫所建立之自主管理架構，可支持橫流溪棲地連通工程之流域生態系管理
• 大甲溪流域石虎族群（本計畫調查範圍）與橫流溪廊道保育密切相關

管理應用：
• 橫流溪工程設施改善應與大甲溪流域整體保育自主管理目標一致
• 若需引用流域尺度野生動物棲地資料，可參考本計畫112年調查成果
• 保育自主管理架構的建立，有助於橫流溪周邊農友配合友善耕作` },

  { id:'friendly_farming_114', kw:['大甲溪流域','東勢','山城社區','友善環境','友善農業','石虎棲地','農業輔導','114年','友善農業認證','農友','生態農業','保育農業','OI值石虎'],
    title:'114年臺中市大甲溪流域東勢周圍山城社區友善環境產業輔導計畫（二）成果報告',
    body:`資料來源：臺中市大甲溪流域東勢周圍山城社區友善環境產業輔導計畫2+成果報告.pdf（246頁）
委託單位：農業部林業及自然保育署臺中分署（東勢處）
執行單位：社企整合股份有限公司
計畫主持人：呂家S（社會企業推動石虎友善農業）
完成時間：114年6月4日（民國114年 = 2025年）
計畫起始：111年（第一期）→ 延續為第二期

113年度計畫成效指標（正式報告數值）：
• 輔導農友農家數：4區域（大安溪、大甲溪東勢周圍、大里市、南投縣）
• 農業社群參與：累計153人次（含農民、社區居民、NGO）
• 在地農年：本地農2人、非本地農1人
• 地方年度農民農村受惠人次：2,000人
• 石虎認證耕作面積：9.177公頃（石虎友善認證標準）
• 農業收入增加：200萬元（透過友善認證品牌行銷增加）

主要工作內容：
1. 生態農業友善棲地調查：3處「石虎友善農耕認證」推廣區域生態調查
2. 農友輔導培訓：共6場工作坊，涵蓋生態農業概念、農法改善、品牌行銷
3. 保育推廣活動：石虎友善農業評鑑大會（113年10月24日舉辦）
4. 農生合作組織發展：協助建立農友自主管理制度
5. 友善農業品牌推廣：結合農業生產與石虎保育宣傳

與橫流溪之關聯：
• 東勢周圍山城社區覆蓋大甲溪東勢段，橫流溪為大甲溪支流，兩者生態廊道相連
• 橫流溪沿岸農業用地若推廣友善耕作，可直接提升石虎廊道品質
• 石虎友善認證（9.177公頃）可作為橫流溪鄰近農業用地保育效益參考基準
• 農友參與保育計畫，有助於降低橫流溪工程施工期間對石虎棲地的干擾

管理建議：
• 橫流溪工程巡查人員宜了解當地農友友善耕作情況，避免施工影響農耕時程
• 若石虎自動相機於橫流溪附近記錄到活動，應通報東勢處協調後續保育措施` }
];
  ...
  try {
    ...
    if (isFishQuery && (q.includes('保育') || q.includes('受脅') || q.includes('紅皮書') ||
      ...
      parts.push(
        ...
        '短吻紅斑吻鰕虎=一般(NLC，惟IUCN全球評估近危NT)。八種均為台灣特有種。' +
...
` +
          `  計算明細：基礎健康分數＝100−(D×0.45+E×0.2+R×0.35)×21，限[15,95]；A1依平台規則校正為健康90分；風險分＝100−健康分數。
` +
...
function buildAllSectionsContext(query) {
  const parts = [];
  const q = (query || '').toLowerCase();
  ...
  try {
    ...
    const FT = {
      'general_periodic':'一般巡查', 'general':'一般巡查',
    ...
    console.warn('[buildAllSectionsContext]', e.message);
  }
  return parts.join('\n');
}
...
【平台5大功能選單（掌握資料來源對應）】
1.工程設施管理：20座設施基本資料、DER&U健康評估、狀態（正常/需維護/損壞）、里程位置、最後巡查日期
2.工程設計書架：歷年工程設計書、成果報告、施工圖說（本機知識庫已內建關鍵內容）
3.生態資料庫：魚類8種（含年度電捕尾次）、水棲昆蟲FBI水質評估、陸域生態、濱溪植物
4.巡查資料管理：一般巡查（general_periodic）、專業構造物巡查（professional_structure）、魚道檢核表（professional_fishway）三類表單
5.維護管理資料：維護完工回報（maintenance_completion），含DER&U、施工前現況、施工方法、完工狀況、後續追蹤、填表人員、填表單位、填表時間、最多4張照片
回答時請說明資料來源屬於哪個選單，並以最新巡查紀錄為準。`;
  ...
  try {
    ...
    if (fishSummary.length > 0) {
      ...
      const fishEntry = {
        ...
        kw: ['魚類','調查','捕獲','尾數','物種','白甲魚','石魚賓','鬚鱲','鰕虎','纓口臺鰍','間爬岩鰍','短臀瘋鱨','短吻紅斑吻鰕虎','明潭吻鰕虎'],
        weight: 2
      };
  ...
  try {
    if (typeof DB !== "undefined") {
      ...
      const recent = fishOnlyQuery ? '' : [...inspections]
        ...
        .slice(0, 5)
        .map(r => {
          const typeLabel = _FT_LABEL[r.formType] || r.inspectionItem || r.sourceType || r.formType || "巡查";
          const deruStr = r.deru_u != null ? ` D${r.deru_d||0}/E${r.deru_e||0}/R${r.deru_r||0} U${r.deru_u}` : '';
          return `${r.date||"-"}｜${r.facilityName||r.facility_name||"-"}｜${typeLabel}${deruStr}｜${r.status||"-"}｜${String(r.findings||r.action||"").slice(0,80)}`;
        })
        .join("\n");
...
// ══════════════════════════════════════════════════════════════════
//  (已移除：雲端文件庫全文搜尋 Modal 已於 2026-08-22 移除)
// ══════════════════════════════════════════════════════════════════

function aiOpenDocSearch() {
  console.info("[AI Chat] 雲端文件庫功能已移除");
  if (typeof showToast === "function") showToast("雲端文件庫功能已移除", "info");

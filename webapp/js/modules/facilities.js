...
function fac_inferDeruFromInspection(item = {}) {
  ...
  //  功能性淤積：結構本體評為良好（D=0）但現場有土砂淤積時，
  //  依指示將淤積反映在評分上，但等級仍以設施結構為主 ——
  //  故不動 D（結構劣化程度），改抬升 E（效能／範圍影響）至 2。
  //  結果：等級維持 A1（d<=0 且 u<=1），健康分數則由 90 降至 84，
  //  使「結構良好但通行效能受影響」如實反映，不致被 A1 的分數下限掩蓋。
  if (d <= 0 && e <= 1 &&
      /淤積|淤塞|堆積|阻塞|堵塞/.test(_cleanText) &&
      !/清淤|已清除|完成改善|疏濬/.test(_cleanText)) {
    e = 2;
    source = source + '（結構良好，另計功能性淤積）';
  }
